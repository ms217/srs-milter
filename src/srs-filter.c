...
xxfi_srs_milter_envrcpt(SMFICTX* ctx, char** argv) {
  ...
  if (!is_local_addr(recip)) {
    ...
    if (recip[0] != '\0' && recip[1] != '\0' 
      ...
      if (!cd->recip) {
        cd->recip = (char **) malloc((nrecip+2)*sizeof(char *));
      } else {
        while (cd->recip[nrecip]) nrecip++;
        cd->recip = (char **) realloc(cd->recip, (nrecip+2)*sizeof(char *));
      }
      ...
      if (!cd->recip) {
        ...
        cd->recip[nrecip] = strdup(recip);
        cd->recip[nrecip+1] = NULL;
        if (!cd->recip[nrecip]) {
          // memory allocation problem
          cd->state |= SS_STATE_INVALID_MSG;
        }
